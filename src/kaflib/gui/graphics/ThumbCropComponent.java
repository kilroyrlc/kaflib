...
import kaflib.gui.components.ImagePanel;
 ...
 */
public class ThumbCropComponent extends DownscaledImageComponent {
	...
	private final void parseBoxes(final String serial) throws Exception {
		try {
			...
			if (serial != null && serial.length() > 0) {
				...
				if (dimensions != null && dimensions.getThumbnail() != null) {
			...
			System.err.println("Error parsing dimensions:\n" + serial);
			throw e;
		}
	}
...
//					update(GraphicsUtils.getCropped(getSourceBufferedImage(), box));
	...
	public void mouseReleased(MouseEvent e) {
		try {
			...
			if (e.getButton() == MouseButton.LEFT.getEvent()) {
			...
			else if (e.getButton() == MouseButton.RIGHT.getEvent()) {
				Worker worker = new Worker(){
					@Override
					protected void process() throws Exception {
	...
	public static void main(String args[]) {
		try {
			...
			final ThumbCropComponent component = new ThumbCropComponent(new File("resource/flag.jpg"), string);
